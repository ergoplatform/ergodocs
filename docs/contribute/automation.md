...
The workflow creates draft pull requests for `draft-pr-safe` outcomes and for `needs-human-review` outcomes when the model returns usable Markdown. If the model sees a likely docs gap but cannot draft a safe page edit, the workflow opens or updates a review issue instead of creating a mergeable note-only PR. `no-doc-change` is the only model decision that skips review output entirely. Draft PR bodies include the model, action, confidence, reviewer uncertainty, and source evidence links. Reruns reuse the same per-page branch and refresh the existing PR body instead of creating duplicates. Before pushing a generated branch, the workflow runs whitespace, nav, structure, and MkDocs build checks. Every PR or review issue is labelled `docs`, `automated`, and `needs-human-review`; protocol, node, contract, ErgoScript, and tutorial pages also receive `sensitive`.
