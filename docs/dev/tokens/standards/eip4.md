...
Note, that additional registers (R4-R6) are encoded as [**Coll[Byte]**](types.md) type of ErgoScript and their encoded representation is received as `'\x0e' + intToVlq(byteArray.length) + byteArray` where `byteArray` is UTF-8 representation of the original string.
